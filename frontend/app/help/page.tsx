...
export default function HelpPage() {
  ...
  const [articles, setArticles] = useState<HelpSection[] | null>(null);
  ...
  useEffect(() => { if (!token || user?.is_superuser) return; fetch(`/api/help?role=${role}`, { headers: { Authorization: `Bearer ${token}` } }).then((response) => response.ok ? response.json() : []).then((data) => setArticles(data.length ? data.map((article: { title: string; body: string }) => ({ title: article.title, body: article.body })) : null)).catch(() => setArticles(null)); }, [role, token, user?.is_superuser]);

  if (loading || !user) return null;
  if (user.is_superuser && token) return <AdminHelpManager token={token} />;
  const content = { ...CONTENT[role], sections: articles ?? CONTENT[role].sections };
